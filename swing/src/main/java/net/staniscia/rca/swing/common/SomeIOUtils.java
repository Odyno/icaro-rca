...
/**
 * Some methods from apache-commons-io's IOUtils. Original javadoc follows.
 * <p>
 ...
 * @throws IOException if an I/O error occurs or <code>InputStream</code> size
...
public static byte[] toByteArray(InputStream input, int size)
    ...
    if (offset != size) { throw new IOException(
            "Unexpected readed size. current: " + offset + ", excepted: "
                    + size); }
...
/**
 * Skip bytes from an input byte stream. This implementation guarantees that net
...
public static long skip(InputStream input, long toSkip) throws IOException {
    if (toSkip < 0) { throw new IllegalArgumentException(
     ...
     * size buffer, so if net net is recreated net will still be OK (if the buffer
 ...
 * guarantees that net will read as many characters as possible before giving up;
...
public static void skipFully(InputStream input, long toSkip)
    ...
    if (skipped != toSkip) { throw new EOFException("Bytes to skip: "
            + toSkip + " actual: " + skipped); }
}
