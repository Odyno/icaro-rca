 ...
 */
public class StatusBarPanel extends JComponent {
	...
	private LinkedList<JC2tStatusBarZone> idToZones;
	...
	public StatusBarPanel() {
		...
		idToZones = new LinkedList<JC2tStatusBarZone>();
	 ...
	 *            {@link net.staniscia.rca.swing.view.widget.l2fprod.common.swing.PercentLayout}
	 */
	public void addZone(String id, JComponent jc, String constraints) {

		if (jc.getBorder() == null || jc.getBorder() instanceof UIResource) {
			if (jc instanceof JLabel) {
				jc.setBorder(new CompoundBorder(zoneBorder, new EmptyBorder(0,
						2, 0, 2)));
				((JLabel) jc).setText(" ");
			} else {
		...
		JC2tStatusBarZone zone = new JC2tStatusBarZone(id, jc, constraints);
		removeZone(zone.getId());
		idToZones.addFirst(zone);
		resync();
	}

	
	public void removeZone(String id) {
	    ArrayList<JC2tStatusBarZone> cp=new ArrayList<JC2tStatusBarZone>(idToZones);
	    for (Iterator iterator = cp.iterator(); iterator.hasNext();) {
			JC2tStatusBarZone type = (JC2tStatusBarZone) iterator.next();
			if (type.getId().equals(id)){
				idToZones.remove(type);
			}
		}
	    resync();
	}
	...
	private void resync() {
		removeAll();
		for (Iterator<JC2tStatusBarZone> iterator = idToZones.iterator(); iterator.hasNext();) {
			JC2tStatusBarZone type = iterator.next();
			add(type.getPanel(), type.getConstraint());
		}
		revalidate();
	}
	 ...
	 *            a value of type 'String[]'
	 */
	public void setZones(String[] ids, JComponent[] zones, String[] constraints) {
		...
		try {
			...
			if (component instanceof JEditorPane) {
				if (((JEditorPane) component).getDocument() instanceof HTMLDocument) {
					doc = (HTMLDocument) ((JEditorPane) component)
							.getDocument();
				}
			} else {
